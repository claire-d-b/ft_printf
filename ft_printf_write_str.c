...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/04 17:18:10 by clde-ber          #+#    #+#             */
/*   Updated: 2020/03/05 18:37:18 by clde-ber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		write_string(int k, char c, const char *str, t_flag *help)
{
	...
	(help->set_prec == 1 && help->precision == -1))) || (str[k + i] &&
	(c == 'p' || c == 'i' || c == 'd' || c == 'x' || c == 'X' || c == 'u')))
	{
		if (help->set_prec == 1 && help->precision == 0 &&
		str[k + i] == '0' && ft_strlen(str) == 1 && c != 's' &&
		help->bool_3 == 0)
			write(1, " ", sizeof(char));
		else if ((str[k + i] == '0' && ft_strlen(str) == 1 && help->set_prec
		== 1 && help->precision == 0 && c != 's') || help->bool_3)
			help->ret--;
		else
...
{
	if (help->set_prec == 1 && help->precision == 0 && str[k] == '0' &&
	ft_strlen(str) == 1 && help->bool_3 == 0)
		write(1, " ", sizeof(char));
