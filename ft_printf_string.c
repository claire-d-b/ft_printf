...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/03 12:10:20 by clde-ber          #+#    #+#             */
/*   Updated: 2020/03/05 18:28:21 by clde-ber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_flags(char c, const char *str, t_flag *help)
{
	...
	help->bool_3 = (str[0] == '0' && ft_strlen(str) == 1 && help->set_prec
	== 1 && help->precision == 0 && help->width < 1 && c != 's' &&
	help->bool_1 == 0) ? 1 : help->bool_3;
	help->zero = ((help->precision > ft_strlen(str) && (c == 'i' || c ==
	'd' || c == 'u' || c == 'p' || c == 'x' || c == 'X'))) ? 1 : help->zero;
	help->precision = (help->precision < 0) ? -1 : help->precision;
	help->set_prec = (help->precision < 0) ? 1 : help->set_prec;
	help->width = (help->width <= ft_strlen(str) && (c == 'i' || c == 'd'
	|| c == 'u' || c == 'p' || c == 'x' || c == 'X')) ? 0 : help->width;
	help->precision = (help->width == ft_strlen(str) && help->precision >
	ft_strlen(str)) ? ft_strlen(str) : help->precision;
	help->precision = (c == 's' && help->precision > ft_strlen(str)) ?
	ft_strlen(str) : help->precision;
	if (c != 's' && help->zero == 1 && help->width > ft_strlen(str) &&
	help->precision == -1)
	{
	...
	help->precision = (c != 's' && str[0] == '-') ? ++help->precision :
	help->precision;
}
	...
	t_flag *help)
{
	...
	help->set_prec == 1 && help->precision != -1 && help->precision <
	help->width)) && c == 's') || (c != 's' && ((j < help->width -
	ft_strlen(str) && (help->set_prec == 0 || (help->set_prec == 1 &&
	(help->precision <= ft_strlen(str) && help->precision != 0)) ||
	(j < help->width - help->precision && help->precision < help->width
...
int		ft_putstr_len_standard(int i, char c, const char *str, t_flag *help)
{
	...
	while (width_len_conditions_standard(++j, c, str, help))
	{
		...
		help->precision == -1)) || help->bool_1)) ?
		k + write_minus(k, c, str, help) : k;
		help->waste = ((help->zero == 1 && (help->set_prec == 0 ||
		help->precision == -1)) || help->bool_1) ? write(1, "0",
		sizeof(char)) : write(1, " ", sizeof(char));
	}
	k += write_minus(k, c, str, help);
	while (++i < help->precision - ft_strlen(str))
	{
		k = (i == 0) ? k + write_address(k, str) : k;
		help->waste = (c != 's') ? write(1, "0", sizeof(char)) : write(1, " ",
		sizeof(char));
	}
	k += write_minus(k, c, str, help);
...
int		ft_putstr_len_rev(int i, char c, const char *str, t_flag *help)
{
	...
	{
		k = (i == 0) ? k + write_address(k, str) : k;
		help->waste = (c != 's') ? write(1, "0", 1) : write(1, " ", 1);
	}
	while (str[k] && c != 's')
	{
...
int		ft_putstr_len(char c, const char *str, t_flag *help)
{
	...
	if (help->rev == 0)
	{
		res = ft_putstr_len_standard(-1, c, str, help);
		if (help->free)
			free((char *)str);
	}
	else
	{
		res = ft_putstr_len_rev(-1, c, str, help);
		if (help->free)
			free((char *)str);
	}
	return (res);
